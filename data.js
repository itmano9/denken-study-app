// 電験三種「理論」学習アプリ — コンテンツデータ(BASE分: 令和5年度上期・令和6年度上期)
// ※ 電気技術者試験センター公式サイトで公開されている実際の過去問PDFから出題内容を確認し、
//    計算過程を検算のうえ収録しています。他の年度は batches/ 配下の各ファイルを参照。
//    最新の過去問や正式な解答は電気技術者試験センターの公式サイトでご確認ください。

const GENRES = [
  {
    id: "dc",
    name: "直流回路",
    desc: "オームの法則・キルヒホッフの法則・合成抵抗・電力",
  },
  {
    id: "electrostatics",
    name: "静電気",
    desc: "クーロンの法則・コンデンサ・静電容量・静電エネルギー",
  },
  {
    id: "electromagnetism",
    name: "電磁気",
    desc: "磁界・電磁力・電磁誘導・自己インダクタンス",
  },
  {
    id: "ac",
    name: "交流回路",
    desc: "RLC回路・インピーダンス・共振・力率・ベクトル図",
  },
  {
    id: "three_phase",
    name: "三相交流",
    desc: "Y結線・Δ結線・線間電圧と相電圧・平衡三相電力・二電力計法",
  },
];

const BASICS = {
  dc: `■ オームの法則
電圧 $V$〔V〕、電流 $I$〔A〕、抵抗 $R$〔Ω〕の間には次の関係が成り立つ。
  $V = IR$　　$I = \\dfrac{V}{R}$　　$R = \\dfrac{V}{I}$

■ 抵抗の直列接続
抵抗を直列につなぐと、合成抵抗はそれぞれの抵抗の和になる。
  $R = R_1 + R_2 + R_3 + \\cdots$
直列回路ではどの抵抗にも同じ電流が流れる。

■ 抵抗の並列接続
抵抗を並列につなぐと、合成抵抗の逆数はそれぞれの抵抗の逆数の和になる。
  $\\dfrac{1}{R} = \\dfrac{1}{R_1} + \\dfrac{1}{R_2} + \\dfrac{1}{R_3} + \\cdots$
並列回路ではどの抵抗にも同じ電圧がかかる。合成抵抗は必ず最も小さい抵抗より小さくなる。

■ キルヒホッフの法則
・電流則(KCL): 回路のどの接続点でも、流れ込む電流の和と流れ出る電流の和は等しい。
・電圧則(KVL): 閉回路を1周すると、起電力の和と電圧降下の和は等しい。
電池に内部抵抗 $r$ がある場合、端子電圧は $V = E - Ir$($E$ は起電力)となる。

■ 電力
抵抗で消費される電力は次のいずれの式でも計算できる。
  $P = VI = I^2 R = \\dfrac{V^2}{R}$

■ 抵抗の温度係数
金属製の抵抗は温度が上がると抵抗値も大きくなる。温度 $t_1$〔℃〕での抵抗を $R_1$、
温度係数を $\\alpha$〔1/℃〕とすると、温度 $t_2$〔℃〕での抵抗 $R_2$ は次の式で求められる。
  $R_2 = R_1\\{1 + \\alpha(t_2 - t_1)\\}$
起電力が一定の回路では、抵抗が大きくなるほど電流 $I = \\dfrac{E}{R}$ は小さくなる。

計算問題を解くときは「まず回路全体の合成抵抗を求める → オームの法則で電流・電圧を求める → 必要なら電力を求める」という順番で考えると整理しやすい。`,

  electrostatics: `■ 静電容量の基本
コンデンサに電圧 $V$〔V〕を加えたとき蓄えられる電荷 $Q$〔C〕と静電容量 $C$〔F〕の関係は次の式で表される。
  $Q = CV$

■ 平行平板コンデンサ
極板面積 $S$〔m²〕、極板間隔 $d$〔m〕、極板間の誘電率 $\\varepsilon$(= $\\varepsilon_0 \\times \\varepsilon_r$、$\\varepsilon_0$は真空の誘電率、$\\varepsilon_r$は比誘電率)とすると、
  $C = \\dfrac{\\varepsilon S}{d} = \\dfrac{\\varepsilon_0 \\varepsilon_r S}{d}$
極板面積が大きいほど、間隔が狭いほど、誘電率(誘電体の種類)が大きいほど静電容量は大きくなる。

■ コンデンサの直列接続
  $\\dfrac{1}{C} = \\dfrac{1}{C_1} + \\dfrac{1}{C_2} + \\cdots$
合成静電容量は必ず最も小さいコンデンサの容量より小さくなる(抵抗の並列と同じ形の式になる点に注意)。

■ コンデンサの並列接続
  $C = C_1 + C_2 + \\cdots$
合成静電容量は単純に足し算になる(抵抗の直列と同じ形の式になる点に注意)。

■ 静電エネルギー
コンデンサに蓄えられるエネルギー $W$〔J〕は次の式で表される。
  $W = \\dfrac{1}{2}CV^2 = \\dfrac{1}{2}QV = \\dfrac{Q^2}{2C}$

抵抗の直列・並列とコンデンサの直列・並列で公式の形が入れ替わる点が混同しやすいので、
「コンデンサの並列 = 抵抗の直列と同じ足し算」「コンデンサの直列 = 抵抗の並列と同じ逆数の和」
と対応づけて覚えると整理しやすい。`,

  electromagnetism: `■ 直線電流による磁界
無限に長い直線状導体に電流 $I$〔A〕が流れているとき、導体から$a$〔m〕離れた点での磁界の強さ$H$〔A/m〕は次の式で表される。
  $H = \\dfrac{I}{2\\pi a}$

■ 円形コイルの中心の磁界
半径$r$〔m〕の円形コイルに電流$I$〔A〕を流したとき、中心の磁界の強さは次の式で表される(1巻きの場合)。
  $H = \\dfrac{I}{2r}$
巻数が$N$のときは$H = \\dfrac{NI}{2r}$となる。また、中心角$\\theta$〔rad〕の円弧状導体による中心の磁界は、全周(角度$2\\pi$)に対する比例配分で
  $H = \\dfrac{I\\theta}{4\\pi r}$
となる($\\theta=2\\pi$を代入すると円形コイルの式に一致することを確認するとよい)。

■ 電磁誘導とファラデーの法則
コイルを貫く磁束$\\Phi$〔Wb〕が時間とともに変化すると、コイルに起電力が発生する(電磁誘導)。巻数$N$のコイルに生じる起電力$e$〔V〕は、向き(符号)まで含めると次の式で表される。
  $e = -N\\dfrac{\\varDelta \\Phi}{\\varDelta t}$
負号はレンツの法則(誘導起電力は、それによって流れる電流が磁束の変化を妨げる向きに生じる)を表している。大きさだけを問われている問題では、絶対値をとって$|e| = N\\dfrac{\\varDelta \\Phi}{\\varDelta t}$として計算すればよい。

■ 自己インダクタンス
コイルに電流$I$〔A〕を流したときに生じる磁束を$\\Phi$〔Wb〕とすると、自己インダクタンス$L$〔H〕は
  $L = \\dfrac{N\\Phi}{I}$
で定義される(すなわち$N\\Phi = LI$)。環状鉄心にコイルを巻いた場合、自己インダクタンスは巻数の2乗に比例する($L \\propto N^2$)。これは、電流が同じでも巻数が$N$倍になると磁束も$N$倍になり、鎖交磁束数$N\\Phi$はさらに$N$倍されるためである。

$N\\Phi = LI$の関係を$e=-N\\dfrac{\\varDelta\\Phi}{\\varDelta t}$に代入すると、自己インダクタンス$L$のコイルに流れる電流$I$が変化するときに生じる自己誘導起電力は、電流と直接次の式で結びつく。
  $e = -L\\dfrac{\\varDelta I}{\\varDelta t}$
この負号も同じくレンツの法則を表しており、電流を増やそうとする(を減らそうとする)向きの変化に対しては、それを妨げる(を保とうとする)向きに起電力が生じることを意味する。大きさだけを求める問題では$|e| = L\\dfrac{\\varDelta I}{\\varDelta t}$とすればよい。

■ 平行導体間に働く力
互いに平行な2本の直線導体にそれぞれ電流を流すと、電磁力(アンペア力)が働く。電流が同じ向きのときは互いに引き合う(吸引力)、逆向きのときは互いに反発する(反発力)。

計算問題では、まず「どの公式(直線電流・円形コイル・電磁誘導・自己インダクタンス)が使える場面か」を見極め、単位(A/m、Wb、H など)を意識しながら式を立てるとよい。`,

  ac: `■ 正弦波交流の基本
瞬時値 $v=V_m\\sin\\omega t$ で表される交流の実効値 $V$ は最大値 $V_m$ の $\\dfrac{1}{\\sqrt{2}}$ 倍になる。
  $V = \\dfrac{V_m}{\\sqrt{2}}$
角周波数 $\\omega$〔rad/s〕と周波数 $f$〔Hz〕の関係は $\\omega = 2\\pi f$。以降の計算では特に断りがなければ電圧・電流は実効値を指す。

■ R・L・Cそれぞれのインピーダンス
・抵抗 $R$〔Ω〕: 電圧と電流は同位相。$\\dot{Z}_R = R$
・誘導性リアクタンス $X_L=\\omega L$〔Ω〕: 電流は電圧より位相が $90°$ 遅れる(電圧が電流より進む)。$\\dot{Z}_L = jX_L = j\\omega L$
・容量性リアクタンス $X_C=\\dfrac{1}{\\omega C}$〔Ω〕: 電流は電圧より位相が $90°$ 進む。$\\dot{Z}_C = -jX_C = -j\\dfrac{1}{\\omega C}$
「コイルは電流が遅れる、コンデンサは電流が進む」という向きを覚えておくと回路の判断がしやすい。

■ RLC直列回路
直列回路のインピーダンス $\\dot{Z}$ は各素子のインピーダンスの和になる。
  $\\dot{Z} = R + j(X_L-X_C)$、　大きさは $Z = \\sqrt{R^2+(X_L-X_C)^2}$
電流 $I=\\dfrac{V}{Z}$ が全素子に共通(直列なので電流基準でベクトル図を描くとよい)。
$X_L>X_C$ のとき回路全体は誘導性(電流が電圧より遅れる)、$X_L<X_C$ のとき容量性(電流が電圧より進む)。

■ RLC並列回路
並列回路はアドミタンス $\\dot{Y}=\\dfrac{1}{\\dot{Z}}$ で考えると見通しがよい。
  $\\dot{Y} = \\dfrac{1}{R} + j\\left(\\dfrac{1}{X_C}-\\dfrac{1}{X_L}\\right)$
各素子の電流を求めて合成するときは、素子間で電圧が共通なのでベクトル図は電圧基準で描く。

■ 直列共振・並列共振
$X_L=X_C$ となる周波数(共振周波数 $f_0$)では、直列回路はインピーダンスが最小($Z=R$)になって電流が最大になり、並列回路はインピーダンスが最大になって電流が最小になる。
  $\\omega_0 L = \\dfrac{1}{\\omega_0 C} \\ \\Rightarrow\\ f_0 = \\dfrac{1}{2\\pi\\sqrt{LC}}$
共振時は電圧・電流が同位相になり、回路は純抵抗として振る舞う。

■ 電力と力率
交流回路の電力には次の3種類がある(電圧・電流は実効値、$\\theta$ は電圧と電流の位相差)。
  皮相電力 $S=VI$〔$\\text{V}\\cdot\\text{A}$〕、有効電力 $P=VI\\cos\\theta$〔W〕、無効電力 $Q=VI\\sin\\theta$〔var〕
$\\cos\\theta$ を力率という。$\\cos\\theta=\\dfrac{R}{Z}=\\dfrac{P}{S}$ の関係があり、純抵抗回路では $\\cos\\theta=1$(力率100%)になる。

計算問題では、まず「直列か並列か」を見極めてインピーダンス(またはアドミタンス)を複素数で立式し、大きさ・位相を求めてから電流・電圧・電力を計算する、という順番で整理するとよい。ベクトル図を描くと位相関係のイメージがつかみやすい。`,

  three_phase: `■ Y結線とΔ結線
三相回路の結線方法には、星形に結ぶY結線(星形結線)と、三角形に結ぶΔ結線(三角結線)がある。
・Y結線: 線間電圧 $V_{\\ell}$ は相電圧 $V_{\\text{p}}$ の $\\sqrt3$ 倍、線電流 $I_{\\ell}$ は相電流 $I_{\\text{p}}$ と等しい。
  $V_{\\ell} = \\sqrt3\\,V_{\\text{p}}$　　$I_{\\ell} = I_{\\text{p}}$
・Δ結線: 線間電圧 $V_{\\ell}$ は相電圧(各辺の電圧) $V_{\\text{p}}$ と等しく、線電流 $I_{\\ell}$ は相電流(各辺を流れる電流) $I_{\\text{p}}$ の $\\sqrt3$ 倍になる。
  $V_{\\ell} = V_{\\text{p}}$　　$I_{\\ell} = \\sqrt3\\,I_{\\text{p}}$
「Y結線は電圧が$\\sqrt3$倍・Δ結線は電流が$\\sqrt3$倍」と対で覚えると整理しやすい。

■ Y-Δ変換(等価変換)
Δ結線の3個のインピーダンス(各辺 $\\dot Z_{\\Delta}$、平衡の場合)は、Y結線に変換すると1相あたり
  $\\dot Z_{\\text{Y}} = \\dfrac{\\dot Z_{\\Delta}}{3}$
になる(逆にYからΔへは $\\dot Z_{\\Delta}=3\\dot Z_{\\text{Y}}$)。不平衡の場合は各辺・各相ごとに一般のΔ-Y変換の式を使う必要があるが、平衡三相回路の計算では上の3倍・$\\frac13$倍の関係だけで十分なことが多い。Y結線に統一して1相分だけを取り出して考えると、単相回路と同じ感覚で解ける。

■ 平衡三相回路の電力
線間電圧 $V$、線電流 $I$、負荷の力率角を $\\theta$ とすると、結線方法によらず三相全体の電力は次の式で表される。
  皮相電力 $S=\\sqrt3\\,VI$〔$\\text{V}\\cdot\\text{A}$〕、有効電力 $P=\\sqrt3\\,VI\\cos\\theta$〔W〕、無効電力 $Q=\\sqrt3\\,VI\\sin\\theta$〔var〕
1相分だけを取り出して $P=3\\times(\\text{1相の電力})$ のように計算しても同じ結果になる(検算に使うとよい)。

■ 対称三相電圧・電流
大きさが等しく、互いに $\\dfrac{2\\pi}{3}\\,\\text{rad}(=120^\\circ)$ ずつ位相がずれた3つの電圧(または電流)の組を対称三相という。平衡三相回路では、線電流のベクトル和は常に0になる($\\dot I_{\\text{a}}+\\dot I_{\\text{b}}+\\dot I_{\\text{c}}=0$)。この関係は中性線に電流が流れないことの根拠にもなり、不平衡回路の一部を求める際にもよく使われる。

■ 二電力計法
三相3線式回路の電力は、2台の単相電力計を使って測定できる(二電力計法)。2台の電力計の指示値 $W_1$、$W_2$(符号付き)の和が三相全体の有効電力になる。
  $P = W_1 + W_2$
負荷の力率が低いと、一方の電力計が逆振れ(負の指示)することがあり、その場合は電圧端子の極性を反転させて絶対値を読み取り、符号をマイナスとして扱う。

計算問題では、まず結線方法(Y or Δ)を確認し、必要ならY結線に統一してから1相分の相電圧・相電流・インピーダンスで考えると整理しやすい。線間電圧と相電圧、線電流と相電流を混同しないよう、常に「今どちらの値を扱っているか」を意識するとよい。`,
};

const BASE_QUESTIONS = [
  // ===== 直流回路(実際の過去問) =====
  {
    genre: "dc",
    yearLabel: "令和6年度上期 問5 / 令和5年度上期 問5(過去問)",
    statement:
      "図の直流回路において、抵抗$R=10\\,\\Omega$で消費される電力の値[W]として、最も近いものは次のうちどれか。",
    diagram: {
      src: "assets/r6f-q5-diagram.png",
      alt: "60Vと80Vの電源、40Ω・40Ω・10Ω(R)・60Ω・60Ωの抵抗からなる直流回路",
      labels: [
        { text: "A", x: 42, y: 19 },
        { text: "B", x: 64, y: 19 },
      ],
    },
    choices: ["0.28", "1.89", "3.79", "5.36", "7.62"],
    answer: 0,
    explanation:
      "節点A・Bの電位をそれぞれ $V_A$、$V_B$(接地を0V)とし、キルヒホッフの電流則を立てる。\n" +
      "節点A: $\\dfrac{60-V_A}{40} = \\dfrac{V_A}{40} + \\dfrac{V_A-V_B}{10}$\n" +
      "節点B: $\\dfrac{V_A-V_B}{10} + \\dfrac{80-V_B}{60} = \\dfrac{V_B}{60}$\n" +
      "これを整理すると $6V_A-4V_B=60$、$6V_A-8V_B=-80$ となり、連立して解くと $V_B=35\\,\\text{V}$、$V_A=\\dfrac{100}{3}\\approx 33.3\\,\\text{V}$。\n" +
      "Rを流れる電流は $I=\\dfrac{V_A-V_B}{10}=\\dfrac{33.3-35}{10}\\approx -0.167\\,\\text{A}$、つまり大きさ約0.167A。\n" +
      "消費電力は $P=I^2R=0.167^2\\times 10\\approx 0.28\\,\\text{W}$。正解は (1) 0.28。",
  },
  {
    genre: "dc",
    yearLabel: "令和6年度上期 問6(過去問)",
    statement:
      "図の回路において、抵抗$R$[Ω]には電流0.3Aが図に示す向きで流れている。抵抗$R$の値[Ω]として、最も近いものは次のうちどれか。",
    diagram: {
      src: "assets/r6f-q6-diagram.png",
      alt: "9V電源、6Ωの抵抗、3V・4Vの電源とR・1Ωの抵抗からなる回路",
      labels: [
        { text: "P", x: 33, y: 16 },
        { text: "Q", x: 77, y: 45 },
      ],
    },
    choices: ["2.0", "2.8", "3.7", "4.9", "25"],
    answer: 0,
    explanation:
      "節点Qを基準(0V)とし、節点Pの電位を$V_P$とする。\n" +
      "上側の枝(P→3V→R→Q、PからQへ0.3A): $V_P + 3 - 0.3R = 0 \\Rightarrow V_P = 0.3R - 3$\n" +
      "下側の枝(P→4V→1Ω→Q、電流を$I_2$とする): $V_P + 4 - I_2\\times 1 = 0 \\Rightarrow I_2 = V_P + 4$\n" +
      "節点Pでの電流則: 電源側から流れ込む電流 $\\dfrac{9-V_P}{6}$ は、上側の0.3Aと下側の$I_2$に分かれる。\n" +
      "$\\dfrac{9-V_P}{6} = 0.3 + I_2$ にそれぞれ代入して整理すると $4.2 = 2.1R$ となり、$R = 2.0\\,\\Omega$。正解は (1) 2.0。",
  },
  {
    genre: "dc",
    yearLabel: "令和5年度上期 問6(過去問)",
    statement:
      "図のような直流回路において、3Ωの抵抗を流れる電流の値[A]として、最も近いものは次のうちどれか。",
    diagram: {
      src: "assets/r5f-q6-diagram.png",
      alt: "4V電源、3Ω・5Ωの抵抗、2Aの電流源からなる回路",
      labels: [{ text: "N", x: 49, y: 10 }],
    },
    choices: ["0.35", "0.45", "0.55", "0.65", "0.75"],
    answer: 4,
    explanation:
      "接地を0V、節点Nの電位を$V_N$とする。3Ωの抵抗を電源からNへ向かう電流を$I_3=\\dfrac{4-V_N}{3}$、5Ωの抵抗をNから接地へ向かう電流を$I_5=\\dfrac{V_N}{5}$とすると、節点Nの電流則より $I_3 + 2 = I_5$。\n" +
      "$\\dfrac{4-V_N}{3} + 2 = \\dfrac{V_N}{5}$ を解くと $V_N = 6.25\\,\\text{V}$。\n" +
      "$I_3 = \\dfrac{4-6.25}{3} = -0.75\\,\\text{A}$、つまり実際には電流はNから電源側へ向かって流れており、大きさは0.75A。正解は (5) 0.75。",
  },
  {
    genre: "dc",
    yearLabel: "令和5年度上期 問7(過去問)",
    statement:
      "図の回路において、スイッチSを閉じ、直流電源から金属製の抵抗に電流を流したとき、発熱により抵抗の温度が120℃になった。スイッチSを閉じた直後(抵抗の温度20℃)に回路を流れる電流に比べ、抵抗の温度が120℃になったときに回路を流れる電流は、どのように変化するか。ただし、抵抗の温度係数は一定で$0.005\\,\\text{℃}^{-1}$とし、電源の起電力の大きさは温度によらず一定、内部抵抗は無視できるものとする。",
    diagram: { src: "assets/r5f-q7-diagram.png", alt: "直流電源、金属製の抵抗、スイッチSからなる単純な直列回路" },
    choices: ["変化しない", "50%増加", "33%減少", "50%減少", "33%増加"],
    answer: 2,
    explanation:
      "抵抗の温度係数の式 $R_2 = R_1\\{1+\\alpha(t_2-t_1)\\}$ より、120℃での抵抗は $R_{120} = R_{20}\\times\\{1+0.005\\times(120-20)\\} = R_{20}\\times(1+0.5) = 1.5\\times R_{20}$。\n" +
      "起電力一定・内部抵抗無視なので $I=\\dfrac{E}{R}$ の関係から、電流は抵抗に反比例して変化する。$\\dfrac{I_{120}}{I_{20}} = \\dfrac{R_{20}}{R_{120}} = \\dfrac{1}{1.5} = \\dfrac{2}{3}$。\n" +
      "つまり電流は元の2/3になる、すなわち約33%減少する。正解は (3) 33%減少。",
  },

  // ===== 静電気(実際の過去問) =====
  {
    genre: "electrostatics",
    yearLabel: "令和6年度上期 問1(過去問)",
    statement:
      "図1に示すような、空気を含む二つの誘電体からなる平行平板電極がある。この下部電極を接地し、上部電極に電圧を加えたときの電極間の等電位線の分布を示す断面図として、正しいものを次の(1)〜(5)のうちから一つ選べ。ただし、誘電体の導電性及び電極と誘電体の端効果は無視できるものとする。参考までに固体誘電体を取り除いた、空気中平行平板電極の場合の等電位線の分布を図2に示す(下側を接地電極とする)。",
    diagram: { src: "assets/r6f-q1-diagram.png", alt: "図1:比誘電率1(空気)と6(固体)の複合誘電体平行平板電極/図2:空気中平行平板電極の等電位線分布" },
    choices: [
      { img: "assets/r6f-q1-c1.png", alt: "選択肢1: 上部から下部まで密な間隔で均一な等電位線" },
      { img: "assets/r6f-q1-c2.png", alt: "選択肢2: 上部から下部まで疎な間隔で均一な等電位線((1)より間隔が広い)" },
      { img: "assets/r6f-q1-c3.png", alt: "選択肢3: 上部が疎(間隔が広い)、下部が密な等電位線" },
      { img: "assets/r6f-q1-c4.png", alt: "選択肢4: 上部が密、下部が疎な等電位線" },
      { img: "assets/r6f-q1-c5.png", alt: "選択肢5: 上部が広い範囲で密、下部のごく一部だけが疎な等電位線" },
    ],
    answer: 4,
    explanation:
      "上部側(比誘電率$\\varepsilon_1=1$の空気層)と下部側(比誘電率$\\varepsilon_2=6$の固体誘電体層)が直列に接続されたコンデンサとみなせる。電束密度$D$は両誘電体の境界で連続(同じ値)だが、電界の強さは$E=\\dfrac{D}{\\varepsilon_0\\varepsilon_r}$の関係にあるため、誘電率が小さい空気層($\\varepsilon_1=1$)の方が電界が強く、誘電率が大きい固体層($\\varepsilon_2=6$)の方が電界は弱くなる(具体的には固体層の電界は空気層の$\\dfrac{1}{6}$)。等電位線の間隔は電界が強い場所ほど狭く(密に)なる。図1では固体誘電体層は下部のごく薄い層で、空気層が大部分を占めているため、密な等電位線が上部の広い範囲(空気層)を占め、下部のごく一部(薄い固体層)だけが疎になる分布が正しい。この分布に一致するのは (5)。",
  },
  {
    genre: "electrostatics",
    yearLabel: "令和6年度上期 問2(過去問)",
    statement:
      "空気中に孤立した半径 $a$[m] の導体球に帯電できる最大の電荷の値[C]として、正しいものは次のうちどれか。ただし、空気の絶縁耐力及び誘電率はそれぞれ $E_m$[V/m] 及び $\\varepsilon_0$[F/m] とする。",
    choices: [
      "$\\dfrac{E_m}{4\\pi\\varepsilon_0 a^2}$",
      "$\\dfrac{E_m}{4\\pi\\varepsilon_0 a}$",
      "$4\\pi\\varepsilon_0 a E_m$",
      "$4\\pi\\varepsilon_0 a^2 E_m$",
      "$4\\pi\\varepsilon_0 a^3 E_m$",
    ],
    answer: 3,
    explanation:
      "半径$a$[m]の導体球表面の電界の強さは $E = \\dfrac{Q}{4\\pi\\varepsilon_0 a^2}$ で表される。この電界が空気の絶縁耐力$E_m$に達したときが帯電できる限界(それ以上では放電してしまう)なので、$E_m = \\dfrac{Q}{4\\pi\\varepsilon_0 a^2}$ より $Q = 4\\pi\\varepsilon_0 a^2 E_m$。正解は (4) $4\\pi\\varepsilon_0 a^2 E_m$。",
  },
  {
    genre: "electrostatics",
    yearLabel: "令和5年度上期 問2(過去問)",
    statement:
      "静電界に関する次の記述のうち、誤っているものを次の(1)〜(5)のうちから一つ選べ。",
    choices: [
      "媒質中に置かれた正電荷から出る電気力線の本数は、その電荷の大きさに比例し、媒質の誘電率に反比例する。",
      "電界中における電気力線は、相互に交差しない。",
      "電界中における電気力線は、等電位面と直交する。",
      "電界中のある点の電気力線の密度は、その点における電界の強さ(大きさ)を表す。",
      "電界中に置かれた導体内部の電界の強さ(大きさ)は、その導体表面の電界の強さ(大きさ)に等しい。",
    ],
    answer: 4,
    explanation:
      "静電界の基本性質として、導体内部は静電誘導によって自由電子が移動し、内部の電界は必ずゼロになる(導体表面には電荷が分布し、そこでの電界はゼロではない)。したがって「導体内部の電界の強さが導体表面の電界の強さに等しい」という記述は誤り。正解は (5)。",
  },
  {
    genre: "electrostatics",
    yearLabel: "令和5年度上期 問1(過去問)",
    statement:
      "電極板面積と電極板間隔が共に$S$[m²]と$d$[m]で、一方は比誘電率が$\\varepsilon_{r1}$の誘電体からなる平行平板コンデンサ$C_1$と、他方は比誘電率が$\\varepsilon_{r2}$の誘電体からなる平行平板コンデンサ$C_2$がある。これらを図のように並列に接続し、端子A、B間に直流電圧$V_0$[V]を加えた。コンデンサ$C_1$の電極板間の電界の強さを$E_1$[V/m]、電束密度を$D_1$[C/m²]、コンデンサ$C_2$の電極板間の電界の強さを$E_2$[V/m]、電束密度を$D_2$[C/m²]とする。$E_1$と$E_2$の組合せ、$D_1$と$D_2$の組合せ、蓄えられる電荷$Q_1$と$Q_2$の組合せとして、正しいものは次のうちどれか。($\\varepsilon_0$は真空の誘電率とする)",
    diagram: { src: "assets/r5f-q1-diagram.png", alt: "比誘電率εr1・εr2の平行平板コンデンサC1・C2を並列接続し電圧V0を加えた回路" },
    choices: [
      "$E_1=\\dfrac{\\varepsilon_{r1}}{d}V_0$, $E_2=\\dfrac{\\varepsilon_{r2}}{d}V_0$ / $D_1=\\dfrac{\\varepsilon_{r1}}{d}SV_0$, $D_2=\\dfrac{\\varepsilon_{r2}}{d}SV_0$ / $Q_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}SV_0$, $Q_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}SV_0$",
      "$E_1=\\dfrac{\\varepsilon_{r1}}{d}V_0$, $E_2=\\dfrac{\\varepsilon_{r2}}{d}V_0$ / $D_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}V_0$, $D_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}V_0$ / $Q_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}SV_0$, $Q_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}SV_0$",
      "$E_1=\\dfrac{V_0}{d}$, $E_2=\\dfrac{V_0}{d}$ / $D_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}SV_0$, $D_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}SV_0$ / $Q_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}V_0$, $Q_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}V_0$",
      "$E_1=\\dfrac{V_0}{d}$, $E_2=\\dfrac{V_0}{d}$ / $D_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}V_0$, $D_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}V_0$ / $Q_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}SV_0$, $Q_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}SV_0$",
      "$E_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}SV_0$, $E_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}SV_0$ / $D_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}}{d}V_0$, $D_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}}{d}V_0$ / $Q_1=\\dfrac{\\varepsilon_0}{d}SV_0$, $Q_2=\\dfrac{\\varepsilon_0}{d}SV_0$",
    ],
    answer: 3,
    explanation:
      "並列接続なので両コンデンサには同じ電圧$V_0$が加わり、極板間隔も共に$d$なので、電界の強さは誘電率によらず $E_1=E_2=\\dfrac{V_0}{d}$ となる(電界は電圧と距離だけで決まる)。\n" +
      "電束密度は $D=\\varepsilon_0\\varepsilon_r\\times E$ なので、$D_1=\\varepsilon_0\\varepsilon_{r1}\\times\\dfrac{V_0}{d}$、$D_2=\\varepsilon_0\\varepsilon_{r2}\\times\\dfrac{V_0}{d}$。\n" +
      "静電容量は $C=\\dfrac{\\varepsilon_0\\varepsilon_r S}{d}$ なので、蓄えられる電荷は $Q=CV_0=\\dfrac{\\varepsilon_0\\varepsilon_r SV_0}{d}$ となり、$Q_1=\\dfrac{\\varepsilon_0\\varepsilon_{r1}SV_0}{d}$、$Q_2=\\dfrac{\\varepsilon_0\\varepsilon_{r2}SV_0}{d}$。\n" +
      "この3つの組合せに一致するのは (4)。",
  },
  {
    genre: "electrostatics",
    yearLabel: "令和6年度上期 問17(a)(過去問)",
    statement:
      "図1の端子a-d間の合成静電容量について、次の(a)及び(b)の問に答えよ。\n" +
      "(a) 端子b-c-d間は図2のようにΔ結線で接続されている。これを図3のようにY結線に変換したとき、電気的に等価となるコンデンサ$C$の値[μF]として、最も近いものを次の(1)~(5)のうちから一つ選べ。",
    diagram: { src: "assets/r6f-q17-fig1-diagram.png", alt: "端子aから9μFを介して節点bへ、端子aから18μFを介して節点cへ接続され、節点bと節点cの間に3μFが接続されている。さらに節点bから3μFを介して端子d側の節点へ、節点cから3μFを介して同じ端子d側の節点へ接続され、その節点がそのまま端子dに繋がっている回路(図1)" },
    choices: ["1.0", "2.0", "4.5", "6.0", "9.0"],
    answer: 4,
    explanation:
      "図2のΔ結線(b-c間、b-d間、c-d間がいずれも$3\\,\\mu\\text{F}$の対称な三角結線)を、図3のY結線(中心点から各端子へ$C$[μF]ずつ)に変換する。\n" +
      "コンデンサの合成則は、直列が抵抗の並列と同じ逆数和、並列が抵抗の直列と同じ足し算になる(静電容量は抵抗ではなくコンダクタンスと同じ振る舞いをする)。したがってΔ-Y変換も、インピーダンス$Z=\\dfrac{1}{j\\omega C}$で考えると、対称な場合は$Z_{\\text{Y}}=\\dfrac{Z_{\\Delta}}{3}$となり、これを静電容量に直すと$C_{\\text{Y}}=3\\times C_{\\Delta}$(抵抗のΔ→Yとは逆に3倍になる点に注意)。\n" +
      "$C_{\\Delta}=3\\,\\mu\\text{F}$なので、$C=3\\times3=9\\,\\mu\\text{F}$。\n" +
      "したがって正解は (5) 9.0。",
  },
  {
    genre: "electrostatics",
    yearLabel: "令和6年度上期 問17(b)(過去問)",
    statement:
      "上記(a)の回路において、図3を用いて図1の端子b-c-d間をY結線回路に変換したとき、図1の端子a-d間の合成静電容量$C_0$の値[μF]として、最も近いものを次の(1)~(5)のうちから一つ選べ。",
    diagram: { src: "assets/r6f-q17-fig23-diagram.png", alt: "図2: 節点b・c・dを頂点とし、b-c間・b-d間・c-d間がいずれも3μFのコンデンサで接続されたΔ結線。図3: 図2のΔ結線をY結線に変換したもので、中心の節点から端子b・c・dへそれぞれC[μF]のコンデンサが接続されている" },
    choices: ["3.0", "4.5", "4.8", "6.0", "9.0"],
    answer: 2,
    explanation:
      "(a)で求めたY結線(中心の節点をOとする)は、O-b間・O-c間・O-d間がいずれも$9\\,\\mu\\text{F}$である。図1の端子aは、bへ$9\\,\\mu\\text{F}$、cへ$18\\,\\mu\\text{F}$で直接つながっているので、これをY結線に置き換えた回路で整理すると次のようになる。\n" +
      "・a→b→O経路: $9\\,\\mu\\text{F}$と$9\\,\\mu\\text{F}$の直列で、$\\dfrac{1}{C}=\\dfrac{1}{9}+\\dfrac{1}{9}=\\dfrac{2}{9}$より$4.5\\,\\mu\\text{F}$。\n" +
      "・a→c→O経路: $18\\,\\mu\\text{F}$と$9\\,\\mu\\text{F}$の直列で、$\\dfrac{1}{C}=\\dfrac{1}{18}+\\dfrac{1}{9}=\\dfrac{1}{18}+\\dfrac{2}{18}=\\dfrac{3}{18}=\\dfrac{1}{6}$より$6.0\\,\\mu\\text{F}$。\n" +
      "この2つの経路はどちらもa-O間を結ぶので並列になり、$C_{\\text{aO}}=4.5+6.0=10.5\\,\\mu\\text{F}$。\n" +
      "最後にO-d間の$9\\,\\mu\\text{F}$と直列になるので、$\\dfrac{1}{C_0}=\\dfrac{1}{10.5}+\\dfrac{1}{9}\\approx0.0952+0.1111=0.2063$、$C_0\\approx4.85\\,\\mu\\text{F}$。\n" +
      "したがって最も近いのは (3) 4.8。",
  },

  // ===== 電磁気(実際の過去問) =====
  {
    genre: "electromagnetism",
    yearLabel: "令和5年度上期 問3(過去問)",
    statement:
      "磁気回路における磁気抵抗に関する次の記述のうち、誤っているものを次の(1)~(5)のうちから一つ選べ。",
    choices: [
      "磁気抵抗は、次の式で表される。 磁気抵抗 = 起磁力 / 磁束",
      "磁気抵抗は、磁路の断面積に比例する。",
      "磁気抵抗は、比透磁率に反比例する。",
      "磁気抵抗は、磁路の長さに比例する。",
      "磁気抵抗の単位は、[H⁻¹]である。",
    ],
    answer: 1,
    explanation:
      "ホプキンソンの法則より、起磁力$NI$と磁束$\\Phi$、磁気抵抗$R_m$の間には$NI=R_m\\Phi$の関係が成り立ち、これを変形すると$R_m=\\dfrac{NI}{\\Phi}=\\dfrac{起磁力}{磁束}$となるので、(1)は正しい。\n" +
      "磁気抵抗は電気回路の抵抗$R=\\dfrac{l}{\\sigma S}$に対応する量で、$R_m=\\dfrac{l}{\\mu S}=\\dfrac{l}{\\mu_0\\mu_r S}$と表される。\n" +
      "この式から、磁気抵抗は磁路の長さ$l$に比例し((4)は正しい)、磁路の断面積$S$には反比例する(「比例する」とする(2)は誤り)。また比透磁率$\\mu_r$には反比例するので(3)は正しい。\n" +
      "さらに$R_m=NI/\\Phi$の単位はA/Wbであり、$1\\,\\text{H}=1\\,\\text{Wb/A}$の関係から$\\text{A/Wb}=\\text{H}^{-1}$となるため(5)も正しい。\n" +
      "したがって誤っているものは (2)。",
  },
  {
    genre: "electromagnetism",
    yearLabel: "令和5年度上期 問4(過去問)",
    statement:
      "磁界及び磁束に関する記述として、誤っているものを次の(1)~(5)のうちから一つ選べ。",
    choices: [
      "1m当たりの巻数が$N$の無限に長いソレノイドに電流$I$[A]を流すと、ソレノイドの内部には磁界$H=NI$[A/m]が生じる。磁界の大きさは、ソレノイドの寸法や内部に存在する物質の種類に影響されない。",
      "均一磁界中において、磁界の方向と直角に置かれた直線状導体に直流電流を流すと、導体には電流の大きさに比例した力が働く。",
      "2本の平行な直線状導体に反対向きの電流を流すと、導体には導体間距離の2乗に反比例した反発力が働く。",
      "フレミングの左手の法則では、親指の向きが導体に働く力の向きを示す。",
      "磁気回路において、透磁率は電気回路の導電率に、磁束は電気回路の電流にそれぞれ対応する。",
    ],
    answer: 2,
    explanation:
      "(1) 無限長ソレノイド内部の磁界は$H=NI$[A/m]で表され、この値はソレノイドの寸法や内部の物質(透磁率)に関わらず一定である(透磁率によって変化するのは磁束密度$B=\\mu H$の方であり、$H$自体は変化しない)。正しい記述。\n" +
      "(2) 電磁力の大きさは$F=BIl\\sin\\theta$で表され、磁界と直角(\\theta=90°)のときは$F=BIl$となり、電流$I$の大きさに比例する。正しい記述。\n" +
      "(3) 平行な2導体間に働く力(単位長さ当たり)は$F=\\dfrac{\\mu_0 I_1 I_2}{2\\pi d}$で表され、導体間距離$d$の1乗に反比例する。「距離の2乗に反比例」というのは誤り。\n" +
      "(4) フレミングの左手の法則では、親指:力、人差し指:磁界、中指:電流の向きをそれぞれ表す。正しい記述。\n" +
      "(5) 磁気回路とオームの法則の対応関係では、起磁力↔起電力、磁束↔電流、磁気抵抗↔抵抗、透磁率↔導電率が対応する。正しい記述。\n" +
      "したがって誤っているものは (3)。",
  },
  {
    genre: "electromagnetism",
    yearLabel: "令和6年度上期 問3(過去問)",
    statement:
      "磁気に関する量とその単位記号(SI基本単位及び組立単位による表し方)の組合せとして、誤っているものを次の(1)~(5)のうちから一つ選べ。",
    choices: [
      "インダクタンス: Wb/A",
      "磁束: V/s",
      "磁界の強さ: A/m",
      "磁気抵抗: H⁻¹",
      "透磁率: H/m",
    ],
    answer: 1,
    explanation:
      "(1) インダクタンス$L$は$L=\\dfrac{N\\Phi}{I}$で定義され、$N\\Phi$の単位はWb、$I$の単位はAなので、$L$の単位はWb/Aとなる(これがHの定義そのもの)。正しい。\n" +
      "(2) 磁束$\\Phi$の単位はWbであるが、ファラデーの法則$e=-N\\dfrac{\\varDelta\\Phi}{\\varDelta t}$より$\\text{V}=\\text{Wb/s}$の関係、すなわち$\\text{Wb}=\\text{V}\\cdot\\text{s}$となる。したがって磁束の単位を「V/s」とするのは誤りで、正しくは「V・s」である。\n" +
      "(3) 磁界の強さ$H$の単位はA/mであり、正しい。\n" +
      "(4) 磁気抵抗$R_m=NI/\\Phi$の単位はA/Wbであり、$\\text{H}=\\text{Wb/A}$の関係から$\\text{A/Wb}=\\text{H}^{-1}$となるので正しい。\n" +
      "(5) 透磁率$\\mu$の単位はH/mであり(真空の透磁率$\\mu_0=4\\pi\\times10^{-7}$H/m)、正しい。\n" +
      "したがって誤っているものは (2)。",
  },
  {
    genre: "electromagnetism",
    yearLabel: "令和6年度上期 問4(過去問)",
    statement:
      "図のように、A、B2本の平行な直線導体があり、導体Aには$1.2\\,\\text{A}$の、導体Bにはそれと反対方向に$3\\,\\text{A}$の電流が流れている。導体AとBの間隔が$l$[m]のとき、導体Aより$0.3\\,\\text{m}$離れた点Pにおける合成磁界が零になった。$l$の値[m]として、最も近いものを次の(1)~(5)のうちから一つ選べ。ただし、導体A、Bは無限長とし、点Pは導体A、Bを含む平面上にあるものとする。",
    diagram: {
      src: "assets/r6f-q4-diagram.png",
      alt: "平行な直線導体AとBがあり、Aには上向きに1.2A、Bには下向きに3Aの電流が流れている。Aの左0.3mの位置に点Pがあり、AB間の距離はl[m]",
    },
    choices: ["0.24", "0.45", "0.54", "0.75", "1.05"],
    answer: 1,
    explanation:
      "点Pにおける導体Aによる磁界の大きさは$H_A=\\dfrac{I_A}{2\\pi\\times0.3}$、導体Bによる磁界の大きさは$H_B=\\dfrac{I_B}{2\\pi(0.3+l)}$で表される。\n" +
      "導体A・Bの電流は互いに逆向きなので、両導体の外側(片側)にある点Pでは右ねじの法則により両者の磁界の向きが逆になり、打ち消し合うことができる。合成磁界がゼロになる条件は、大きさが等しいこと、すなわち$H_A=H_B$である。\n" +
      "$\\dfrac{1.2}{2\\pi\\times0.3}=\\dfrac{3}{2\\pi(0.3+l)}$ の両辺の$2\\pi$を消去すると $\\dfrac{1.2}{0.3}=\\dfrac{3}{0.3+l}$、すなわち$4=\\dfrac{3}{0.3+l}$。\n" +
      "これを解くと $0.3+l=\\dfrac{3}{4}=0.75$、よって$l=0.75-0.3=0.45\\,\\text{m}$。正解は (2) 0.45。",
  },

];
